...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	meanSpeed = meanSpeed * kmhInMsec // м/c
	heightInM := height / cmInM       // рост в метрах
	spentCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return spentCalories
}
